...
export const USAGE = `用法:
  ...
  kimi-agent review-diff [<git range>] [--output <file>]
...
export function precheck(opts) {
  ...
  if (opts.cmd === 'review-diff') {
    ...
    if (!numstat.trim()) throw new UsageError(`范围 ${range} 内没有可 review 的变更`);
  }
}
```
...
`main` 接上最小执行路径（暂只支持 `run`，其余子命令仍 return 0，Task 8 补全）：
...
export async function main(argv) {
  ...
  if (opts.cmd !== 'run') return 0; // Task 8 接入模板子命令
  try {
...
test('review-diff: DIFF_RANGE 嵌入 git diff 命令，diff 不进 prompt', () => {
  const out = renderTemplate(tpl('review-diff.md'), { DIFF_RANGE: 'main..HEAD' });
...
Expected: 打印 kimi 命令行与完整渲染后的 prompt，含"请 review 以下文件：README.md"，无 `{{` 残留。
